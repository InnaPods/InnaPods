...
function Cart(tax = 0.2, shipping = 0) {
    ...
    this.setCartTotal = function(shoppingCartItems) {
        ...
        document.querySelector('.cart-subtotal').textContent = this.totalInCartWithoutTax();
        document.querySelector('.cart-tax').textContent = (this.totalInCartWithoutTax() * this.tax);
        document.querySelector('.cart-shipping').textContent = this.shipping;
        document.querySelector('.cart-total').textContent = (+this.totalInCart() + +this.shipping).toFixed(2);
    }
    // 
    ...
    this.totalInCartWithoutTax = function() {
        let total = 0;
        for (let item in cart) {
    ...
    this.totalInCart = function() {
        ...
        return currency(total + total*(1 * this.tax) + this.shipping);
    }
    ...
    this.productTemplate = (product) => `
        <article class="product" data-id="${product.id}">
            ...
            <div class="image">
                ...
                <img style="width: 192px; height: 240px;" src="${product.image}" alt="${product.name}">
            </div>
            <div class="content" data-id="${product.id}">
                <h3 class="product-name bgc-w">${product.name}</h3>
...
let sectionName = section => {
    ...
    div.setAttribute('class', 'py-2 px-4 bg-dark text-white mb-1 br-50');
...
function main() {
    ...
    const url = "https://my-json-server.typicode.com/InnaPods/db";
        ...
        <div class="container page-footer">
            ...
            <section class="footer-ligal">
                <ul class="footer-ligal-list">
                    ...
                    <li><a href="#">&copy; 2024 Copyright Deco-style Inc.</a></li>
                </ul>
